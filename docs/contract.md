...
A `session` label, among an envelope's free-form extras, names the conversation a turn belongs to. Exactly four kinds carry it — `turn-started`, `turn-activity`, `turn-completed`, `turn-interrupted` — and **no other kind a member or the graph emits carries one**, including `oneharness-session` itself. Its value is the emitting stream's id, a `.`, and the member id — `node-scope-1786925518098-3163646.worker` — sanitised to the consumer's identifier rule: non-empty, at most 128 characters, ASCII letters, digits, `-`, `_` and `.` only, never starting with `.`. Neither id is bounded, so a pair past that length keeps a share of each — the end of the stream, which is what tells two runs of one graph apart, and the start of the member — followed by a `-` and a 16-character digest of the whole pair, so two members of one run are never one conversation. The exclusion is the load-bearing half rather than a detail: a consumer treats any labelled envelope that is not `turn-activity` or `turn-interrupted` as one transcript turn, so a `session` on the thousands of heartbeats one member publishes over a run would render a transcript of heartbeats and make every turn count served from it wrong.
